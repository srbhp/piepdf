...
class Adjust_UI(object):
    ...
    def decorateMain(self):
        ...
        print(self.piepdf_path)
        self.piepdf_db = sqldb.DatabaseInit(
            self.piepdf_path, email=self.parent_ui.emailaddress
        )
    ...
    def addSearchBox(self):
        ...
        pix = QtGui.QIcon.fromTheme("system-search")
        self.searchIcon.setPixmap(pix.pixmap(QtCore.QSize(16, 16)))
        self.searchWidget.setStyleSheet(
            "QLabel {background-color : white; border: 5px solid white; border-top-left-radius: 10px; border-bottom-left-radius: 10px;} QLineEdit {background-color : white;border: 5px solid white; border-top-right-radius: 10px; border-bottom-right-radius: 10px;}"
        )
    ...
    def addsignals(self):
        ...
        self.parent_ui.listView.selectionModel().selectionChanged.connect(
            self.metaSignal
        )
