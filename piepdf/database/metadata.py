...
class GetPdfInfo(object):
    ...
    def getDoiFromPdf(self, filename ):
        ...
        if  doc is not None :
            ...
            self.page0_text = doc.page(0).text(  QtCore.QRectF( min, min,
                rect.width(),rect.height()) ,)#.to_utf8()
            regString =  r"\b(10\.[0-9]{4,}(?:\.[0-9]+)*\/(?:(?![\"&\'])\S)+)\b"
            doi = re.search(regString ,self.page0_text ).group()
    ...
    def parseDoi(self,pageStr ):
        doi=""
        x = re.search(r"\b(10\.[0-9]{4,}(?:\.[0-9]+)*\/(?:(?![\"&\'])\S)+)\b",pageStr)
        #Find doi from url
        try : 
            doi = x.group()[0]
        except :
            print("Unable to find doi from pdf")
            print(pageStr)
        return doi
