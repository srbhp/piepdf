...
piepdf_PATH = QtCore.QStandardPaths.standardLocations(QtCore.QStandardPaths.ConfigLocation)[0] + '/piepdf'
CONFIG_PATH = QtCore.QStandardPaths.standardLocations(QtCore.QStandardPaths.ConfigLocation)[0] + '/piepdf/piepdf.conf'
...
class InfoWindow(QtWidgets.QWidget ):
    ...
    def dataChangeSignal(self):
        ...
        with sqlite3.connect(self.database_path) as db :
            cursor = db.cursor()
            cursor.execute("UPDATE pdffulltext SET {} = ? WHERE path = ? ".format(key),(value,self.metadata['Path']))
