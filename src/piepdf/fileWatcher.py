...
class FileWatcher(QtWidgets.QMainWindow):
    def __init__(self, pathToWatch=None, parent=None):
        ...
        self.fileList = [
            os.path.join(dp, f) for dp, dn, fn in os.walk(self.pathToWatch) for f in fn
        ]
    ...
    def slotDirChanged(self, path):
        ...
        chFiles = xor(tempFile, self.fileList)
        chFolders = xor(self.dirList, tempDir)
        msg = ""
...
def main():
    ...
    window = FileWatcher("/home/saurabh/Dropbox/github/work/test")
    window.show()
    window.raise_()
